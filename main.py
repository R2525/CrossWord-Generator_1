...
# 임의 단어 리스트
letterList =["start" ,"aab" ,"bbc" ,"caacd" ,"dabwwde" ,"ewaea" ,"ssdabdnas" ,"sudo" ,"wobasdh" ,"dibdm" ,"hiss"
              ,"alskasms" ,"insites" ,"dasfsdf" ,"dihwanalsnd" ,"ipnwin," ,"dlka" ,"nsdnwin" ,"aoihgda" ,"sbdkanf"
              ,"ohsdfus" ,"dfgbds" ,"gjmsdwudg" ,"vasxn" ,"bczxnfvi" ,"prothjusyh" ,"gfxzhcxznf" ,"mneifhji" ,"osehfznb"
              ,"cvmzxbcz" ,"vxcyqaf" ,"dyuwe" ,"bqofeop" ,"tjproy" ,"dfgbmcvb" ,"cvnbaks" ,"fgasbeyau" ,"fgsdf"
              ,"ojibwre" ,"jfgasnn" ,"vkjdaas" ,"vgfyuv" ,"usaf" ,"hlgfadn" ,"hbfadjgv" ,"hiacyo" ,"cckjgl" ,"ksdkjt"
              ,"yopas" ,"dlkneao" ,"hlmxc" ,"vihea" ,"ljkb" ,"nzdx"]
letterList2 =["start"]
...
def WordReader_H(letter, hor_locate, ver_locate):
    ...
    able = 0
    global tryednum
    if '!' in array2[ver_locate, hor_locate] or "x" in array2[p_Y[-1], p_X[-1] - 1] or "x" in array2[
        p_Y[-1], p_X[-1] + len(letter)] or "!" in array2[p_Y[-1], p_X[-1] - 1] or "!" in array2[
        p_Y[-1], p_X[-1] + len(letter)]:
        return able == 0
    for x in letter:
        if '!' in array2[ver_locate, hor_locate] or "x" in array2[p_Y[-1], p_X[-1] - 1] or "x" in array2[
            p_Y[-1], p_X[-1] + len(letter)] or "!" in array2[p_Y[-1], p_X[-1] - 1] or "!" in array2[
...
def WordReader_V(letter, hor_locate, ver_locate):
    ...
    for x in letter:
        ...
        if '!' in array2[ver_locate, hor_locate] or "x" in array2[p_Y[-1] - 1, p_X[-1]] or "x" in array2[
            p_Y[-1] + len(letter), p_X[-1]] or "!" in array2[p_Y[-1] - 1, p_X[-1]] or "!" in array2[
            p_Y[-1] + len(letter), p_X[-1]]:
            print("XXXXXXXXXXXXXXXXXXXXXXXX")
            cando.append(1)
